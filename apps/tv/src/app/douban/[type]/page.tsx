...
function DoubanPageClient({ type }: { type: string }) {
        ...
        if (weekdayData) {
          data = {
            ...
            list: weekdayData.items.map((item) => ({
              ...
              poster:
                // 卡片场景优先小图（common≈180px），避免冷缓存经代理拉 1-3s/3MB 原图
                item.images.common ||
                item.images.medium ||
                item.images.small ||
                item.images.large ||
 ...
 const getActivePath = () => {
  ...
  return (
      ...
      <div className='px-3 sm:px-8 py-2 sm:py-4 overflow-visible'>
        {/* 页面标题和选择器 */}
        <div className='mx-auto mb-4 sm:mb-5 max-w-[92%] 2xl:max-w-[1320px] space-y-2.5 sm:space-y-3'>
          {/* 页面标题 */}
        ...
        {/* 内容展示区域 */}
        <div className='max-w-[92%] 2xl:max-w-[1320px] mx-auto mt-4 overflow-visible'>
          {/* 内容网格 */}
          {loading || !selectorsReady
            ? // 显示骨架屏
            <div className='justify-start grid grid-cols-3 gap-x-2 gap-y-6 px-0 sm:px-2 sm:grid-cols-[repeat(auto-fill,minmax(128px,1fr))] sm:gap-x-4 sm:gap-y-8'>
              {skeletonData.map((index) => <DoubanCardSkeleton key={index} />)}
            </div>
            : // 显示实际数据
            <VirtualGrid
              items={doubanData}
              className='grid-cols-3 gap-x-2 px-0 sm:px-2 sm:grid-cols-[repeat(auto-fill,minmax(128px,1fr))] sm:gap-x-4'
...
export default function DoubanPage({ params }: { params: { type: string } }) {
  return (
    <Suspense>
      <DoubanPageClient type={params.type} />
    </Suspense>
  );
}
