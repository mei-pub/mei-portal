...
// 播放页客户端实现（从 page.tsx 抽出）：Next 页面模块只允许导出默认页组件与
// 特定配置字段，命名导出 PlayPageClient 会触发 .next/types 的入口类型检查报错。
// 路由页（/play 与 /play/[source]/[id]）从这里引入并各自包 Suspense 边界。
...
export function PlayPageClient({ pathSource, pathId }: { pathSource?: string; pathId?: string } = {}) {
 ...
 const [currentSource, setCurrentSource] = useState(
  ...
  const handleDownloadToServer = useCallback(async () => {
    ...
    // 播放页当前路由（usePathname 不含 /tv basePath）：done 后管理页据此跳回完整播放器
    const qs = searchParams.toString();
    const playRoute = pathname ? (qs ? `${pathname}?${qs}` : pathname) : undefined;
    setLocalBusy(true);
    setLocalError(null);
    try {
                  ...
                  className={`ml-2 flex-shrink-0 transition-all ${
                    currentLocalRecord?.status === 'done'
                      ...
                      : currentLocalRecord?.status === 'failed'
                        ...
                        : currentLocalRecord?.status === 'pending' ||
                            currentLocalRecord?.status === 'downloading'
                          ? 'rounded-full bg-blue-500/10 px-3 py-1 text-xs font-semibold text-blue-600 hover:bg-blue-500/20 dark:text-blue-400'
