...
const statusTag = (record: MovieSourceRecord) => {
  switch (record.status) {
    ...
    case "failed":
      return <DownloadTag icon={<FailedIcon />} text="失败" color="#ff7373" />;
    default:
...
const MoviePanel: FC = () => {
  ...
  const handleDeleteRecord = useMemoizedFn(
    async (record: MovieSourceRecord) => {
  ...
  const renderRow = (record: MovieSourceRecord) => {
    ...
    return (
      ...
      >
        ...
        {statusTag(record)}
        {record.status === "done" &&
          (record.playRoute || movieFallbackVideo(record)) && (
            <IconButton
              title={t("playVideo")}
      ...
      return (
        ...
        >
          <div className="flex flex-row items-center gap-2">
            ...
            {category && <DownloadTag text={category} color="#6366f1" />}
