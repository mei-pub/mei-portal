// 闪电影视（Shandian）插件 —— Go plugin/shandian/shandian.go 的复刻
// 三镜像（MacCMS 模板）搜索页 → 详情页抓取 UC 云盘链接（本源只收 uc）。
// 说明：Go 的 HTTP 连接池参数、性能统计原子计数器、缓存清理协程未移植（与搜索语义无关，
//       见报告）；detailCache 以模块级 Map 等价复刻（无淘汰）。
...
const PLUGIN_NAME = 'shandian';

const SOURCE_URLS = ['http://sduc.cloud', 'http://shandian.blog', 'http://sd.sduc.site'];
...
// UC云盘链接正则（Go ucLinkRegex，含可选查询串）
...
/** 校验是否为有效网盘URL（Go isValidNetworkDriveURL：本源只认 UC 云盘） */
...
/** 获取详情页的下载链接（Go fetchDetailLinks）；失败静默返回空 */
async function fetchDetailLinks(base: string, itemID: string): Promise<Link[]> {
  const trimmed = base.replace(/\/+$/, '');
  ...
  $('#download-list .module-row-one').each((_i, el) => {
    ...
    if (linkURL && isValidNetworkDriveURL(linkURL)) {
      links.push({ type: 'uc', url: linkURL, password: '' }); // UC云盘通常不需要密码
    }
    ...
    row.find('a[href]').each((_j, a) => {
      ...
      links.push({ type: 'uc', url: hrefURL, password: '' });
    });
  });
...
async function enhanceWithDetails(results: SearchResult[], baseURL: string): Promise<SearchResult[]> {
  ...
  return Promise.all(
    results.map(async (result) => {
      // 从 UniqueID 提取 itemID（"shandian-<id>"）
      ...
      const links = await fetchDetailLinks(baseURL, itemID);
      const enhancedResult: SearchResult = { ...result, links };
...
export const shandian = definePlugin({
  name: PLUGIN_NAME,
  priority: 2, // Go NewBaseAsyncPlugin("shandian", 2)
