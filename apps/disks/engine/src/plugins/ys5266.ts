// 66影视（5266YS）磁力搜索插件 —— Go plugin/5266ys/5266ys.go 的复刻
// 站点 www.5266ys.net，帝国 CMS，GB18030 编码：POST 搜索表单（keyword GB18030 编码）
// → 列表 .listBox li → 详情页提取磁力链接、简介与剧照。
// 注意：JS/TS 标识符不能以数字开头，导出名用 ys5266，插件名字符串保持 "5266ys"。
...
const PLUGIN_NAME = '5266ys';
const BASE_URL = 'https://www.5266ys.net';
...
async function fetchSearchDoc(keyword: string): Promise<Doc> {
  ...
  const form = 'show=title%2Csmalltext&tempid=1&tbname=article&keyboard=' + queryEscapeBytes(encoded) + '&submit=';
...
async function fetchDetail(detailURL: string): Promise<DetailResult> {
  ...
  try {
    ...
    let contentNode = $('.contentinfo').first();
    if (contentNode.length === 0) contentNode = $('.mainleft').first();
...
/** 解析列表项（.listBox li → h3 a / a[href]） */
function parseSearchItems($: Doc): SearchItem[] {
  const items: SearchItem[] = [];
  const seen = new Set<string>();
  $('.listBox li').each((_i, li) => {
    const s = $(li);
    let anchor = s.find('h3 a').first();
    if (anchor.length === 0) anchor = s.find('a[href]').first();
    ...
    const id = DETAIL_ID_RE.exec(fullURL)?.[1] ?? '';
    items.push({ id, title, detailURL: fullURL, datetime: parseDateText(s.text()) });
  });
  return items;
}
...
/** 第一张内容剧照（#text img, .contentinfo img, #dede_content img, img；跳过 logo） */
function firstContentImage($: Doc): string {
  let imageURL = '';
  $('#text img, .contentinfo img, #dede_content img, img').each((_i, img) => {
...
export const ys5266 = definePlugin({
  name: PLUGIN_NAME,
  priority: 3, // Go NewBaseAsyncPluginWithFilter("5266ys", 3, true)
      ...
      items.map((item) =>
        limit(async (): Promise<SearchResult[]> => {
          ...
          detail.magnets.forEach((magnet, index) => {
            ...
            let magnetID = magnetHash(magnet.url);
            if (magnetID === '') {
              magnetID = String(index);
            } else {
              magnetID = `${magnetID}-${fnv32hex(magnet.url)}`;
            }
            let title = item.title;
            if (title === '') title = '未知影片';
            if (magnet.subtitle !== '') title += ' - ' + magnet.subtitle;
            const content = detail.content !== '' ? detail.content : '来源：66影视';

            const result: SearchResult = {
              message_id: `${PLUGIN_NAME}-${id}-${magnetID}`,
              unique_id: `${PLUGIN_NAME}-${id}-${magnetID}`,
