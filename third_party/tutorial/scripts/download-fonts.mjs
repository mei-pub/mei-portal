/**
 * 字体预下载脚本 (纯 JS 版本，无 TypeScript 依赖)
 * 从 Google Fonts CDN 下载字体 CSS 和字体文件到本地 data/fonts/ 目录。
 * 运行方式: node scripts/download-fonts.mjs
 * 
 * 下载完成后，/api/fonts 路由将纯从本地文件提供服务，不访问外网。
 * 已存在的字体会自动跳过，只下载缺失的字体。
 */
...
async function fetchFromCDNs(cssPath) {
...
async function downloadLXGWWenKai() {
  ...
  if (fs.existsSync(localPath) && fs.existsSync(cssFilePath)) {
...
async function main() {
  ...
  for (const [family, cssPath] of Object.entries(FONT_FAMILIES)) {
    ...
    // Check if already downloaded — skip if CSS and all font files exist
    ...
    // Special handling for LXGW WenKai
    if (cssPath === 'jsdelivr-lxgw') {
      const hadCss = fs.existsSync(cssFilePath);
  ...
  console.log(`跳过: ${skipCount}  新下载: ${successCount}  失败: ${failCount}`);
  console.log(`CSS files: ${cssFiles.length}, Font files: ${fontFiles.length}, Total: ${Math.round(totalSize / 1024 / 1024 * 10) / 10} MB`);

  if (failCount > 0) {
    console.log('\n⚠ 部分字体下载失败，容器启动后可重新运行: node scripts/download-fonts.mjs');
  }
}
