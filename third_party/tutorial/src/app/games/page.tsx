...
export default function GamesPage() {
  const [spiderDialog, setSpiderDialog] = useState(false);
  const [pokerDialog, setPokerDialog] = useState(false);
  
  // 蜘蛛纸牌设置
  const [spiderSuits, setSpiderSuits] = useState(1);
  const [spiderNickname, setSpiderNickname] = useState("");
  ...
  function startSpiderGame() {
    if (!spiderNickname.trim()) return;
    window.location.href = `/games/spider?suit=${spiderSuits}&nickname=${encodeURIComponent(spiderNickname)}`;
  }
  ...
  return (
    <>
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-4 sm:px-6 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl border border-[var(--border)] overflow-hidden shadow-sm hover:shadow-md transition-shadow">
            ...
            <div className="p-6">
              <h2 className="text-xl font-bold mb-2">蜘蛛纸牌</h2>
              <p className="text-sm text-[var(--muted)] mb-4">
                经典单人纸牌游戏，将所有纸牌按从 K 到 A 的顺序排列完成。
              </p>
              <div className="flex items-center gap-2 text-xs text-[var(--muted)] mb-4">
                <span className="px-2 py-1 bg-green-100 text-green-700 rounded">单人</span>
                <span>支持存档</span>
              </div>
              <button
                onClick={() => setSpiderDialog(true)}
      ...
      {/* 蜘蛛纸牌对话框 */}
      {spiderDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setSpiderDialog(false)}>
          <div className="relative bg-white rounded-xl shadow-2xl p-6 w-full max-w-sm mx-4" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-lg font-bold text-gray-900">蜘蛛纸牌</h2>
              <button onClick={() => setSpiderDialog(false)} className="text-gray-400 hover:text-gray-600 text-xl leading-none">&times;</button>
            </div>
            <div className="space-y-4">
              <div>
                ...
                <input
                  type="text"
                  value={spiderNickname}
                  onChange={e => setSpiderNickname(e.target.value)}
                  onKeyDown={e => { if (e.key === "Enter") startSpiderGame(); }}
              ...
              <div className="flex gap-2 pt-2">
                <button 
                  onClick={startSpiderGame} 
                  disabled={!spiderNickname.trim()}
                  className="flex-1 py-2 bg-green-600 text-white text-sm font-medium rounded-md hover:bg-green-700 transition-colors disabled:opacity-50"
                >
                  开始游戏
                </button>
                <button onClick={() => setSpiderDialog(false)} className="px-4 py-2 text-sm text-gray-500 border border-gray-200 rounded-md hover:bg-gray-50 transition-colors">
                  取消
                </button>
              </div>
            </div>
